...
> Phase 6.7 code was merged in PR #8 (squash commit `db1a249`).
> The execution artifact was produced on 2026-04-15 on Ash Khan's
> local MacBook Pro (MACS2 2.2.9.1). All authoritative values in
> this file are filled from the run's `.meta.json`. See
> `REAL_DATA_BLOCKERS.md` → "Artifacts produced" →
...
Checklist satisfied during the 2026-04-15 run:
1. ✅ `macs2 --version` → 2.2.9.1
2. ✅ `bash scripts/download_pbmc10k_multiome.sh` exits 0 with all three hashes verified.
3. ✅ `python scripts/harmonize_peaks.py ... --genome hg38` exits 0 with 115720 peaks (above the 50,000 sanity gate).
4. ✅ Idempotency preserved: script refuses overwrite without `--force`; deterministic sort-before-write.
5. ✅ stdout values copied verbatim into REAL_DATA_BLOCKERS.md "Artifacts produced" and into this file.
...
- Committing the peak-set artifact itself — it remains gitignored
  by design; its SHA-256 and `.meta.json` are the reproducibility
  contract.
