...
Format: tab-separated, peaks as rows, samples as columns.
First column = peak coordinate (chr:start-end). Subsequent columns =
sample IDs encoding {donor}-{cell_type}-{stim_state}.
...
Outputs an AnnData with:
    ...
    - .var = chrom, start, end, peak_id (chr:start-end)
...
# Calderon sample IDs follow the pattern: {donor}-{cell_type}-{stim_state}
# e.g. "1001-CD4_Teff-S" (stimulated), "1002-Bulk_B-U" (unstimulated).
...
def parse_sample_ids_to_metadata(sample_ids: list[str]) -> pd.DataFrame:
    """Extract donor / cell_type / stim_state from column headers."""
    rows = []
...
PEAK_RE = re.compile(r"^(?P<chrom>chr[\w]+):(?P<start>\d+)-(?P<end>\d+)$")
...
def parse_calderon_counts(counts_path: Path) -> tuple[sp.csr_matrix, pd.DataFrame, list[str]]:
    ...
    X : scipy.sparse.csr_matrix
        (n_samples, n_peaks) int32 counts.
    var_df : pd.DataFrame
        Indexed by peak_id (chr:start-end), columns chrom/start/end.
    sample_ids : list[str]
    ...
    with _open_counts(counts_path) as fh:
        ...
        if len(sample_ids) == 0:
            raise ValueError(
                f"Calderon header has no sample columns: {header[:5]!r}"
            )
...
def main():
    ...
    p.add_argument("--counts", required=True, type=Path,
                   help="Path to GSE118189_ATAC_counts.txt.gz")
