...
OUT_PATH = REPO / "docs" / "deck" / "exports" / "aivc_investor_4slide_v1_1.pptx"
...
def _phase_panel(slide, left, header_label, header_color, header_subtitle, when_text,
    ...
    # v1.1 — italic tagline removed (overlapped Panel 2/3 encoder hexagons in v1).
    # Per surgical-fix spec: delete uniformly across all 3 panels. The
    # `italic_caption` argument is accepted for call-site compatibility but
    # intentionally unused — taglines were the weakest content on each panel
    # (marketing language, not investor signal).
    _ = italic_caption
